...
### List of unmatched (STEBSUM)

```{r}

# find where stebs_full$verbatim_name is not in cc_names$stebsum_name
obs_not_in_STEBSUM <- stebs_full %>%
  anti_join(cc_names, by = c("verbatim_name" = "stebsum_name")) %>%
  select(verbatim_name, date) %>%
  distinct() %>%
  as_tibble()
...
all_data <- all_data %>% # get ordinal date and flight min/max
  mutate(ord_date = as.numeric(format(date, "%j"))) %>%
  group_by(genus_species, dataset) %>%
  mutate(flight_min = as.numeric(min(ord_date)), 
         flight_max = as.numeric(max(ord_date))) %>%
  ungroup()

# create an object with all collection dates by dataset
dates_by_dataset <- all_data %>% 
  group_by(ord_date, dataset, year) %>%
  summarize()

# create an object with flight windows for each species
flight_windows <- all_data %>%
  group_by(genus_species, flight_min, flight_max, dataset) %>%
  summarize()

# davis only subset
flight_windows_davis <- flight_windows %>%
  filter(dataset == "davis")

# davis only subset
dates_by_dataset_davis <- dates_by_dataset %>%
  filter(dataset == "davis")

# find whether date is within flight window for each species
for (i in 1:nrow(flight_windows_davis)) {
  y <- as.vector(as.matrix(flight_windows_davis[i, 1]))
  dates_by_dataset_davis <- dates_by_dataset_davis %>%
    mutate(!!y := ifelse(
      between(
        ord_date,
        flight_windows_davis$flight_min
        [which(flight_windows_davis$genus_species == y)],
        flight_windows_davis$flight_max
        [which(flight_windows_davis$genus_species == y)]
      ),
      1,
      0
...
# make df with collection dates for davis
species_collection_dates_davis <- dates_by_dataset_davis %>%
  ungroup() %>%
  select(!(c(ord_date, dataset))) %>%
  group_by(year) %>%
...
# stebs only subset
flight_windows_stebs <- flight_windows %>%
  ungroup() %>%
  filter(dataset == "stebs")

# stebs only subset
dates_by_dataset_stebs <- dates_by_dataset %>%
  ungroup() %>%
  filter(dataset == "stebs")

# find whether date is within flight window for each species
for (i in 1:nrow(flight_windows_stebs)) {
  y <- as.vector(as.matrix(flight_windows_stebs[i, 1]))
  dates_by_dataset_stebs <- dates_by_dataset_stebs %>%
    mutate(!!y := ifelse(
      between(
        ord_date,
        flight_windows_stebs$flight_min[which(flight_windows_stebs$genus_species == y)],
        flight_windows_stebs$flight_max[which(flight_windows_stebs$genus_species == y)]
      ),
      1,
      0
    ))
...
# make df with collection dates for stebs
species_collection_dates_stebs <- dates_by_dataset_stebs %>%
  ungroup() %>%
  select(!(c(ord_date, dataset))) %>%
  group_by(year) %>%
  summarise(across(everything(), ~ sum(., is.na(.), 0))) %>%
  ...
  as.data.frame() %>%
  rownames_to_column(var = "genus_species") %>%
  mutate(dataset = "stebs")

# join dfs together
species_collection_dates <-
full_join(species_collection_dates_stebs,
species_collection_dates_davis)

# pivot df longer
species_collection_dates <- species_collection_dates %>%
  pivot_longer(!c(genus_species, dataset),
               names_to = "year",
               values_to = "n_collection_dates_adj") %>%
  mutate(year = as.numeric(year))

all_data <- left_join(all_data,
          species_collection_dates,
          by = c("genus_species", "year", "dataset"))

# collect metadata columns only
species_metadata <- all_data %>%
  distinct(genus_species, dataset, .keep_all = TRUE) %>%
  select(
    genus_species,
    dataset,
    genus,
...
all_data <- all_data %>% # get number of collection dates per year
  group_by(year, dataset) %>% 
  mutate(n_collection_dates_all = n_distinct(date)) %>%
  ungroup()

# collect year, collection dates, species, and dataset
year_metadata <- all_data %>% 
  select(year, dataset, n_collection_dates_all) %>%
  distinct() %>%
  right_join(., species_collection_dates)

# get "day positives" per species per year
species_positives <- all_data %>%
  group_by(genus_species, year, dataset) %>% 
  mutate(day_positives = n_distinct(date)) %>%
  select(year, dataset, genus_species, day_positives) %>%
  distinct()

# join data frames together and fix day_positives = 0
all_data_zeros <- year_metadata %>%
  left_join(., species_positives) %>%
  ungroup() %>%
  mutate(day_positives = ifelse(
    n_collection_dates_adj != 0 &
      is.na(day_positives),
    0,
    day_positives
  )) %>%
  filter(!c(dataset == "stebs" & year == c(1998))) %>%
  filter(!c(dataset == "davis" & year == c(1989:1997))) %>%
  ungroup() %>%
  left_join(., species_metadata, by = c("genus_species", "dataset")) %>%
  mutate(day_negatives = n_collection_dates_adj - day_positives)
